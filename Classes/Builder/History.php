<?php
namespace JournalTransporterPlugin\Builder;
...
class History
{
    ...
    public function toEventArray()
    {
        ...
        foreach ($this->events as $event) {
            ...
            $out[] = (object)[
                'date' => Date::formatDateString($logEntry['date_logged']),
                'event' => $logEntry['event_title'],
        ...
        foreach ($milestones as $milestone) {
            if ($eventTitle === $milestone->event) {
                if (!is_null($messageRegex)) {
                    if (preg_match($messageRegex, $milestone->message)) {
